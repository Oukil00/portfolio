...
import { FaExternalLinkAlt, FaFigma, FaDribbble } from 'react-icons/fa'; // Import FaDribbble icon
      ...
      <div className="px-3 py-5 mx-auto text-center sm:mx-6 md:mx-12 md:pt-5 md:mt-5 xl:mx-40">
        ...
        <div className="mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-12 sm:gap-x-3 md:gap-x-5 lg:gap-x-2 lg:gap-y-5 xl:gap-y-10 xl:gap-x-5 mt-4 md:mt-8">
          {ProjectsData.map((project) => (
            <div data-aos="zoom-in-up" data-aos-duration="1000" data-aos-once="false" key={project.id} className="group relative flex flex-col flex-wrap mx-auto shadow-xl rounded-xl">
              <img 
                src={project.image} 
                alt={project.name} 
                ...
                <div data-aos="zoom-in-up" data-aos-duration="1000" data-aos-once="false" className="flex gap-7 justify-center items-center my-7 text-2xl">
                  {project?.figma && (
                    <div className="bg-white rounded-full p-2">
                      <a className="text-darkblue text-xl bg-white" href={project.figma} target="_blank" rel="noopener noreferrer">
                        <FaFigma className='p-[1px]' />
                      </a>
                    </div>
                  )}
                  {project?.demo && (
                  ...
                  {project?.dribbble && ( // Check for dribbble link
                    <div className="bg-white rounded-full p-2">
                      <a className="text-darkblue text-xl bg-white" href={project.dribbble} target="_blank" rel="noopener noreferrer">
                        <FaDribbble className='p-[1px]'/>
                      </a>
                    </div>
                  )}
                </div>
